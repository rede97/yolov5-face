...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='./weights/yolov5n-face-relu.pt', help='weights path')  # from yolov5/models/
    ...
    if opt.onnx_infer:
        ...
        for i in range(len(y_onnx)):
            print(f"name: {output_names[i]}, pred's shape[{i}] is ",y_onnx[i].shape)
            print("max(|torch_pred - onnx_pred|） =",abs(y[i].cpu().numpy()-y_onnx[i]).max())
